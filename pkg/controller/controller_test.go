...
// newTestController builds a Controller backed by a fake clientset, suitable
// for unit-testing controller methods without a real cluster. Tests may
// further customize the returned Controller's fields (opts, cfg, gpuNodes,
// etc.) before invoking the method under test. The returned buffer captures
// everything written to c.output.
func newTestController(client kubernetes.Interface) (*Controller, *bytes.Buffer) {
...
func TestAddJob(t *testing.T) {
	...
	if len(c.jobs) != 0 {
		t.Fatalf("expected no jobs initially, got %d", len(c.jobs))
	}

	j := jobrunner.Job(nil)
	c.AddJob(j)
	if len(c.jobs) != 1 {
		t.Fatalf("expected 1 job after AddJob, got %d", len(c.jobs))
	}
}
...
// Tests for pingmesh classification, rail-mapping, and BW-probe topology
// reconciliation now live in pkg/checks/rdma (pingmesh_classify_test.go,
// topology_report_test.go), next to the code they exercise.
